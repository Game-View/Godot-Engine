...
func _physics_process(delta):
	...
	if is_dragging and not (is_rotating or is_scaling):
		...
		# First, get the object's current 2D screen position.
		var object_screen_pos = camera.unproject_position(global_transform.origin)
		
		# Calculate the difference between the mouse's position and the object's screen position.
		var drag_delta = last_mouse_pos - object_screen_pos
		
		# Calculate the distance between the mouse and the object on the screen.
		var distance = drag_delta.length()
		
		# Only apply force if the distance is greater than our screen_stop_distance.
		if distance > screen_stop_distance:
			# Get the camera's global transformation.
			var camera_transform = camera.global_transform
			
			# Get the camera's "right" (X-axis) and "up" (Y-axis) vectors.
			...
			# Normalize the vector if it's not zero to get a clean direction.
			if movement_direction.length() > 0:
				movement_direction = movement_direction.normalized()
			
			
			# Apply a force to the RigidBody3D in the calculated direction.
			target_node.apply_central_force(movement_direction * attraction_strength*delta)
	elif is_dragging and is_scaling:
			# If the object is within the stop distance, stop its movement entirely.
		target_node.linear_velocity = Vector3.ZERO
		target_node.angular_velocity = Vector3.ZERO
	elif is_dragging and is_rotating:
		target_node.linear_velocity = Vector3.ZERO
