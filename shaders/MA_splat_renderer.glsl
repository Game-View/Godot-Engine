...
layout(std140, binding = 0) uniform CameraMatrices {
    mat4 viewMat; // Camera to world inverse (view matrix)
    mat4 projMat; // Projection matrix
    vec4 viewport; // Viewport parameters (x, y, width, height)
    vec4 projParams; // Projection parameters (width, near, far, 0.0)
    vec3 eye; // Camera position in world space
};
// Vertex inputs with layout qualifiers
layout(location = 0) in vec4 position; // Center of Gaussian (alpha in w)
layout(location = 1) in vec4 r_sh0; // Red SH coefficients (order 0)
layout(location = 2) in vec4 g_sh0; // Green SH coefficients (order 0)
layout(location = 3) in vec4 b_sh0; // Blue SH coefficients (order 0)
#ifdef FULL_SH
layout(location = 4) in vec4 r_sh1; // Red SH coefficients (order 1)
layout(location = 5) in vec4 r_sh2; // Red SH coefficients (order 2)
layout(location = 6) in vec4 r_sh3; // Red SH coefficients (order 3)
layout(location = 7) in vec4 g_sh1; // Green SH coefficients (order 1)
layout(location = 8) in vec4 g_sh2; // Green SH coefficients (order 2)
layout(location = 9) in vec4 g_sh3; // Green SH coefficients (order 3)
layout(location = 10) in vec4 b_sh1; // Blue SH coefficients (order 1)
layout(location = 11) in vec4 b_sh2; // Blue SH coefficients (order 2)
layout(location = 12) in vec4 b_sh3; // Blue SH coefficients (order 3)
layout(location = 13) in vec3 cov3_col0; // Covariance matrix column 0
layout(location = 14) in vec3 cov3_col1; // Covariance matrix column 1
layout(location = 15) in vec3 cov3_col2; // Covariance matrix column 2
#else
layout(location = 4) in vec3 cov3_col0; // Covariance matrix column 0
layout(location = 5) in vec3 cov3_col1; // Covariance matrix column 1
layout(location = 6) in vec3 cov3_col2; // Covariance matrix column 2
#endif

// Outputs to fragment shader
out vec4 frag_color; // Radiance and alpha of splat
out vec4 frag_cov2inv;
// Inverse 2D screen-space covariance matrix
out vec2 frag_p; // 2D screen-space center of Gaussian
...
void vertex() {
    ...
    vec4 t = viewMat * vec4(position.xyz, 1.0);
    // Viewport dimensions
